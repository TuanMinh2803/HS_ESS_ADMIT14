#include<math.h>
// #include "LSM9DS1_KF.h" change took too much effort, use header from original library, added functions and vars for kalman filter calculation
...
//ReadAccel line409 to calcMag line 484

///////////////////////////////////////////

//ReadX functions for Kalman filter calculation

void LSM9DS1_Read_Accel(I2C_HandleTypeDef *I2Cx, LSM9DS1_t *DataStruct){
	uint8_t Rec_Data[6];

	//Read 6 Bytes of data from Accel register

	if(xgReadBytes(OUT_X_L_XL, Rec_Data, 6) == 6){
		DataStruct->Accel_X_RAW = (Rec_Data[1]<<8) | Rec_Data[0];  //Store raw x-axis accel to memory
		DataStruct->Accel_Y_RAW = (Rec_Data[3]<<8) | Rec_Data[2];  //Store raw x-axis accel to memory
		DataStruct->Accel_Z_RAW = (Rec_Data[5]<<8) | Rec_Data[4];  //Store raw x-axis accel to memory
	}

	DataStruct->Ax = calcAccel(DataStruct->Accel_X_RAW);
	DataStruct->Ay = calcAccel(DataStruct->Accel_Y_RAW);
	DataStruct->Az = calcAccel(DataStruct->Accel_Z_RAW);
}

void LSM9DS1_Read_Gyro(I2C_HandleTypeDef *I2Cx, LSM9DS1_t *DataStruct){
	uint8_t Rec_Data[6];

	//Read 6 Bytes of data from Gyro register

	if(xgReadBytes(OUT_X_L_G, Rec_Data, 6) == 6){
		DataStruct->Gyro_X_RAW = (Rec_Data[1]<<8) | Rec_Data[0];  //Store raw x-axis accel to memory
		DataStruct->Gyro_Y_RAW = (Rec_Data[3]<<8) | Rec_Data[2];  //Store raw x-axis accel to memory
		DataStruct->Gyro_Z_RAW = (Rec_Data[5]<<8) | Rec_Data[4];  //Store raw x-axis accel to memory
	}

	DataStruct->Gx = calcGyro(DataStruct->Gyro_X_RAW);
	DataStruct->Gy = calcGyro(DataStruct->Gyro_Y_RAW);
	DataStruct->Gz = calcGyro(DataStruct->Gyro_Z_RAW);
}

void LSM9DS1_Read_All(I2C_HandleTypeDef *I2Cx, LSM9DS1_t *DataStruct){
	uint8_t Rec_Data1[6], Rec_Data2[6];
	int16_t temp;

	//Read 14 byte of data

	if(xgReadBytes(OUT_X_L_XL, Rec_Data1, 6) == 6){
			DataStruct->Accel_X_RAW = (Rec_Data1[1]<<8) | Rec_Data1[0];  //Store raw x-axis accel to memory
			DataStruct->Accel_Y_RAW = (Rec_Data1[3]<<8) | Rec_Data1[2];  //Store raw y-axis accel to memory
			DataStruct->Accel_Z_RAW = (Rec_Data1[5]<<8) | Rec_Data1[4];  //Store raw z-axis accel to memory
		}

	if(xgReadBytes(OUT_X_L_G, Rec_Data2, 6) == 6){
			DataStruct->Gyro_X_RAW = (Rec_Data2[1]<<8) | Rec_Data2[0];  //Store raw x-axis gyro to memory
			DataStruct->Gyro_Y_RAW = (Rec_Data2[3]<<8) | Rec_Data2[2];  //Store raw y-axis gyro to memory
			DataStruct->Gyro_Z_RAW = (Rec_Data2[5]<<8) | Rec_Data2[4];  //Store raw z-axis gyro to memory
	}
