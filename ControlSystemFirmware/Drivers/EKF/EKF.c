...
void EKF_Predict(EKF *ekf, float p_rps, float q_rps, float r_rps, float SampleTime_s){
	...
	float dphi_dt   = p_rps +tt * (q_rps * sp + r_rps * cp);
	float dtheta_dt = 			  (q_rps * cp - r_rps * sp);

	//Update state estimates (x(n+1) = x(n)+ T*dx/dt)

	ekf->phi_r 	 += SampleTime_s * dphi_dt;
	ekf->theta_r += SampleTime_s * dtheta_dt;
	...
	//Calculate Jacobian of state transition function A(x,u) = df(u,x)/dt

	float A[2][2] = {		{   tt * (q_rps * cp - r_rps * sp)		,		(q_rps * sp + r_rps * cp) * ctInv * ctInv},
	...
	ekf->P[0][0] += SampleTime_s * P_new[0][0];		ekf->P[0][1] += SampleTime_s * P_new[0][1];
	ekf->P[1][0] += SampleTime_s * P_new[1][0];		ekf->P[1][1] += SampleTime_s * P_new[1][1];
}
...
void EKF_Update (EKF *ekf, float ax_mps2, float ay_mps2, float az_mps2){
	...
	//Compute Kalman gain K = P * C' * (R + C * P * C')^-1 in steps (C[0][0] = 0)
	...
	float detMatInv = 1.0f /	(	RCPCt[0][0] * (RCPCt[2][2] * RCPCt[1][1] - RCPCt[2][1] * RCPCt[1][2]   )
								  - RCPCt[1][0] * (RCPCt[2][2] * RCPCt[0][1] - RCPCt[2][1] * RCPCt[0][2]   )			//Calculate 1/det(Matrix)
	...
	//Update state covariance matric P(n+1) = (I - K * C) * P(n)
	...
	float Pnew[2][2] = {	{IminKC[0][0] * ekf->P[0][0] + IminKC[0][1] * ekf->P[1][0]	, 	IminKC[0][0] * ekf->P[0][1] + IminKC[1][0] * ekf->P[1][1]	},
							{IminKC[1][0] * ekf->P[0][0] + IminKC[1][1] * ekf->P[1][0]  ,   IminKC[1][0] * ekf->P[0][1] + IminKC[1][1] * ekf->P[1][1]	}      };

	//Assign new P matrix to ekf struct
